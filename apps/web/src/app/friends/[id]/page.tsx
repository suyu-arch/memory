...
export default async function FriendPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="page">
    ...
    {Object.entries(grouped).sort(([a],[b])=>Number(b)-Number(a)).map(([year,items]) => <section key={year}><h2 className="year-label">{year}</h2><div className="timeline">{items?.map((encounter) => <Link href={`/encounters/${encounter.id}`} className="memory-card" key={encounter.id}><div className="date-tile"><strong>{String(new Date(encounter.startAt).getDate()).padStart(2,'0')}</strong><span>{String(new Date(encounter.startAt).getMonth()+1).padStart(2,'0')}月</span></div><div><h3>{encounter.title}</h3><p>{encounter.story}</p></div><div className="memory-meta">{encounter.locationText}<br/>{encounter.photoCount} 张照片</div></Link>)}</div></section>)}
  </div>;
}
